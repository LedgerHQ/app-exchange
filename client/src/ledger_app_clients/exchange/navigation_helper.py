...
class ExchangeNavigationHelper:
    ...
    def _navigate_and_compare(self, accept: bool):
        ...
        # Don't try to assert the "Processing" spinner if validated
    ...
    def wait_for_library_spinner(self):
        # Handle applications not yet ported on NBGL for Nano
        if self._backend.firmware.is_nano:
            try:
                self._backend.wait_for_text_on_screen("Signing", timeout=2.0)
            except TimeoutError:
                pass
