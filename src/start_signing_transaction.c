...
int start_signing_transaction(const command_t *cmd) {
    ...
    // Create the variable given to the signing app. It's placed in stack to be able to be shared
    // with the lib application
    ...
    create_payin_transaction(&lib_in_out_params, &reported_result);
    if (reported_result) {
        G_swap_ctx->state = SIGN_SUCCESS;
    } else {
        G_swap_ctx->state = SIGN_FAILURE;
    }
