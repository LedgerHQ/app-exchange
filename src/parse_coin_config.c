...
int parse_coin_config(unsigned char *config, unsigned char config_length,                        //
    ...
    *pure_config_length = 0;
    if (config_length < 3) return 0;
    *ticker_length = config[0];
    if (config_length < 3 + *ticker_length) return 0;
    if (*ticker_length > 0) *ticker = config + 1;
    *application_name_length = config[1 + *ticker_length];
    if (config_length < 3 + *ticker_length + *application_name_length) return 0;
    if (*application_name_length > 0) *application_name = config + 1 + *ticker_length + 1;
