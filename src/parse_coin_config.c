...
int parse_coin_config(const buf_t *const orig_buffer,
    ...
    // Read ticker
    if (parse_to_sized_buffer(input.bytes, input.size, ticker, &total_read) != 0) {
    ...
    if (parse_to_sized_buffer(input.bytes, input.size, application_name, &total_read) != 0) {
    ...
    // Read configuration
    if (parse_to_sized_buffer(input.bytes, input.size, configuration, &total_read) != 0) {
