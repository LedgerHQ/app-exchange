...
static void initialize_pointer_field(void *pItem, pb_field_iter_t *iter) {
    if (PB_LTYPE(((const pb_field_t *) PIC(iter->pos))->type) == PB_LTYPE_STRING ||
        ...
        *(void **) pItem = NULL;
        ...
        pb_message_set_to_defaults((const pb_field_t *) ((const pb_field_t *) PIC(iter->pos))->ptr,
                                   pItem);
    }
}
#endif
            ...
            if (wire_type == PB_WT_STRING && PB_LTYPE(type) <= PB_LTYPE_LAST_PACKABLE) {
                ...
                while (substream.bytes_left) {
                    ...
                    if ((size_t) *size + 1 > allocated_size) {
                         ...
                         * upwards. */
                        size_t remain = (substream.bytes_left - 1) /
                                            ((const pb_field_t *) PIC(iter->pos))->data_size +
                                        1;
    ...
    if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
        ...
        if (init_data) {
            if (PB_LTYPE(((const pb_field_t *) PIC(iter->pos))->type) == PB_LTYPE_SUBMESSAGE) {
                ...
                memcpy(iter->pData,
                       ((const pb_field_t *) PIC(iter->pos))->ptr,
                       ((const pb_field_t *) PIC(iter->pos))->data_size);
            } else {
                /* Initialize to zeros */
                memset(iter->pData, 0, ((const pb_field_t *) PIC(iter->pos))->data_size);
    ...
    /* Check that all required fields were present. */
    {
        ...
        if (req_field_count > 0) {
            ...
            for (i = 0; i < (req_field_count >> 5); i++) {
                if (fields_seen[i] != allbits) PB_RETURN_ERROR(stream, "missing required field");
            }
...
bool pb_decode_delimited_noinit(pb_istream_t *stream,
                                const pb_field_t fields[],
                                void *dest_struct) {
    pb_istream_t substream;
    bool status;
...
static bool pb_release_union_field(pb_istream_t *stream, pb_field_iter_t *iter) {
     ...
     * invalid data. */
    if (!pb_field_iter_find(iter, old_tag)) PB_RETURN_ERROR(stream, "invalid union tag");

    pb_release_single_field(iter);

    /* Restore iterator to where it should be.
     * This shouldn't fail unless the pb_field_t structure is corrupted. */
    if (!pb_field_iter_find(iter, new_tag)) PB_RETURN_ERROR(stream, "iterator error");
