...
static bool all_received_v2(uint32_t rcv_flags) {
    uint32_t required_flags =
        (get_tag_flag(STRUCT_TYPE) | get_tag_flag(STRUCT_VERSION) |
         get_tag_flag(TRUSTED_NAME_TYPE) | get_tag_flag(TRUSTED_NAME_SOURCE) |
         get_tag_flag(TRUSTED_NAME) | get_tag_flag(CHAIN_ID) | get_tag_flag(ADDRESS) |
         get_tag_flag(SOURCE_CONTRACT) | get_tag_flag(CHALLENGE) | get_tag_flag(SIGNER_KEY_ID) |
         get_tag_flag(SIGNER_ALGO) | get_tag_flag(SIGNATURE));
...
static bool handle_source_contract(const tlv_data_t *data, tlv_out_t *trusted_name_info) {
    return get_cbuf_from_tlv_data(data, &trusted_name_info->spl_token, 0, MAX_ADDRESS_LENGTH);
}

static bool handle_trusted_name(const tlv_data_t *data, tlv_out_t *trusted_name_info) {
    return get_cbuf_from_tlv_data(data, &trusted_name_info->token_account, 1, MAX_ADDRESS_LENGTH);
}
...
static swap_error_e verify_struct(const tlv_out_t *trusted_name_info,
    ...
    switch (trusted_name_info->struct_version) {
        case 2:
            if (!all_received_v2(received_tags_flags)) {
...
static int trusted_name_descriptor_handler_internal(const command_t *cmd) {
    ...
    PRINTF("Token account : %.*H owned by %.*H\n",
           trusted_name_info.token_account.size,
           trusted_name_info.token_account.bytes,
           trusted_name_info.owner.size,
           trusted_name_info.owner.bytes);

    PRINTF("Checking against PAYOUT address\n");
    ...
    PRINTF("Checking against REFUND address\n");
