...
/*
    ...
    unsigned char der_signature[90];
    unsigned char hash[32];
    cx_hash_sha256(input_buffer, sizeof(input_buffer), hash, 32);
    unsigned int info = 0;
    cx_ecdsa_sign(&privKey, CX_LAST | CX_RND_TRNG, CX_SHA256, hash, 32, der_signature, 90, &info);
    if (os_memcmp(original_der_signature, der_signature, 70) != 0) {
        PRINTF("NOT EQ!!!");
        return;
    }
    if (cx_ecdsa_verify(&ctx->ledger_public_key, CX_LAST, CX_SHA256, hash, 32, original_der_signature, 70) == 0) {
...
*/
   unsigned char rawKey[32];
    ...
    cx_hash_sha256(input_buffer, sizeof(input_buffer), hash, 32);
    unsigned int info = 0;
    if (privKey.curve != CX_CURVE_SECP256K1) {
        PRINTF("%d", privKey.curve);
        return;
    }
