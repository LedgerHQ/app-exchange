...
test('Test btc signed', async () => {
  ...
  const new_transport: Transport<string> = await HttpTransport.open("http://127.0.0.1:9998");
  var 
  ans = await new_transport.send(0xe0, 0x40, 0x00, 0x00, Buffer.from("058000005480000000800000000000000000000000", "hex"));

  ans = await new_transport.send(0xe0, 0x44, 0x00, 0x02, Buffer.from("0100000002", "hex"));
  ...
  ans = await new_transport.send(0xe0, 0x4a, 0x00, 0x00, Buffer.from("0220a107000000000017a914b860f421a99ca42bc540c02d5bc5e8fecd6ffcf487be120800000000001600140f16966f7a3c", "hex"));
