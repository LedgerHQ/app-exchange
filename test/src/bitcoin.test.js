...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Wrong payout address should be rejected`, zemu(model, async (sim) => {
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Valid Bitcoin payout address should be accepted`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Wrong Bitcoin refund address should not be accepted`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Valid refund address should be accepted`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
        ...
        await sim.navigateAndCompareSnapshots('.', `${model.name}_btc_valid_refund_address_is_accepted`, [4, 0]);
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Valid taproot refund address should be accepted`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
        ...
        // Wait until we are not in the main menu
        await waitForAppScreen(sim);
        await sim.navigateAndCompareSnapshots('.', `${model.name}_btc_valid_taproot_refund_address_is_accepted`, [4, 0]);
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Overflow values should be trimmed when swapping`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
        ...
        // Wait until we are not in the main menu
        await waitForAppScreen(sim);
        await sim.navigateAndCompareSnapshots('.', `${model.name}_btc_to_btc_overload_swap`, [4, 0]);
        await expect(checkRequest).resolves.toBe(undefined);
    }))
});
