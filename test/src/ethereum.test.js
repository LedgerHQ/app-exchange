...
test('[Nano S] Wrong Ethereum payout address should not be accepted', zemu(nano_environments[0], async (sim) => {
...
test('[Nano S] Valid Ethereum payout address should be accepted', zemu(nano_environments[0], async (sim) => {
    const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
    const transactionId: string = await swap.startNewTransaction();
    await swap.setPartnerKey(partnerSerializedNameAndPubKey);
    await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
...
test('[Nano S] Wrong Ethereum refund address should not be accepted', zemu(nano_environments[0], async (sim) => {
    const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
    const transactionId: string = await swap.startNewTransaction();
    await swap.setPartnerKey(partnerSerializedNameAndPubKey);
    await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
