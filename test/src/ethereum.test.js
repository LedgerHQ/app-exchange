...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Wrong Ethereum payout address should not be accepted`, zemu(model, async (sim) => {
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Valid Ethereum payout address should be accepted`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
...
nano_environments.forEach(function(model) {
    test(`[Nano ${model.letter}] Wrong Ethereum refund address should not be accepted`, zemu(model, async (sim) => {
        const swap = new Exchange(sim.getTransport(), TRANSACTION_TYPES.SWAP);
        const transactionId: string = await swap.startNewTransaction();
        await swap.setPartnerKey(partnerSerializedNameAndPubKey);
        await swap.checkPartner(DERSignatureOfPartnerNameAndPublicKey);
