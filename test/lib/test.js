...
test('Test BTC swap to LTC fails', async () => {
  ...
  try {
    var ans = await transport.send(0xe0, 0x44, 0x00, 0x02, Buffer.from('0100000002', 'hex')); // nVersion + number of inputs
    ...
    ans = await transport.send(0xe0, 0x4a, 0x80, 0x00, Buffer.from('356BC3BF6FB0E7555F67688A377D', 'hex')); // start signing inputs
...
test('Test ETH swap to BTC', async () => {
  jest.setTimeout(100000);
  let transport = await getTransport(); //const transport: Transport<string> = await TransportNodeHid.open();
  ...
  tr.setCurrencyTo("BTC"); // 1 ETH to 1 BTC
  ...
  transport = await getTransport(); //transport = await TransportNodeHid.open();
...
test('Test Aeternity ERC20 swap to BTC', async () => {
  ...
  tr.setCurrencyTo("BTC"); // 1.1234 AE to 1 BTC
  ...
  const aeInfo = (0, _erc.byContractAddress)("0x5CA9a71B1d01849C0a95490Cc00559717fCF0D1d");
