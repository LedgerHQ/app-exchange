...
DOT_PACKED_TRANSACTION_SIGN_LAST_CHUNK = bytes([0x05, 0x00, 0x00, 0xb0, 0x0b, 0x9f, 0x27, 0xc2, 0xd1, 0xd2, 0x16,
                                                ...
                                                0x00, 0x90, 0x24, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00, 0x91, 0xb1,
...
DOT_PACKED_TRANSACTION_SIGN_LAST_CHUNK_WRONG_DEST_ADDR = bytes([0x05, 0x00, 0x00, 0xdc, 0x5a, 0xda, 0x10, 0xee, 0xdd, 0x89, 0x81, 0x92,
                                                                ...
                                                                0x70, 0xf9, 0xdb, 0xdf, 0x02, 0x45, 0x01, 0x00, 0x00, 0x90, 0x24, 0x00,
                                                                0x00, 0x13, 0x00, 0x00, 0x00, 0x91, 0xb1, 0x71, 0xbb, 0x15, 0x8e, 0x2d,
...
DOT_PACKED_TRANSACTION_SIGN_LAST_CHUNK_WRONG_METHOD = bytes([0x05, 0x02, 0x00, 0xdc, 0x5a, 0xda, 0x10, 0xee, 0xdd, 0x89, 0x81, 0x92, 0x78,
                                                             ...
                                                             0x90, 0x24, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00, 0x91, 0xb1, 0x71, 0xbb, 0x15,
...
def prepare_exchange(backend, firmware, navigator,
    ...
    if(addresses_ok):
        with ex.check_address(payout_signer=LEDGER_SIGNER, refund_signer=LEDGER_SIGNER):
            navigator.navigate_until_text(NavIns(NavInsID.RIGHT_CLICK),
...
testdata = [
    ...
    (b"tz1YPjCVqgimTAPmxZX9egDeTFRCmrTRqmp9", "XTZ", 1000000), # 1 XTZ = 1e6 mutez 
    (b"GCNCEJIAZ5D3APIF5XWAJ3JSSTHM4HPHE7GK3NAB6R6WWSZDB2A2BQ5B", "XLM", 1000000), # 1 XLM = 1e6 stroop
...
def test_swap_flow_dot_nominal(backend, firmware, navigator, payout_addr, curr_to, amount_to_wallet):
    prepare_exchange(backend,firmware, navigator, 
                     payout_addr=payout_addr,curr_to=curr_to,amount_to_wallet=amount_to_wallet)
    dot = PolkadotClient(backend)
    # Get public key.
    key = dot.get_pubkey()
...
def test_swap_flow_dot_refuse_double_sign(backend, navigator, firmware):
    prepare_exchange(backend,
                     firmware,
                     navigator)
    dot = PolkadotClient(backend)
    # Get public key.
    ...
    excinfo_list = ["Connection reset by peer","Max retries exceeded","0 bytes read","Connection aborted"]
    with pytest.raises((ChunkedEncodingError, ConnectionError)) as excinfo:
        dot.sign_init()
    assert any(item in str(excinfo.value) for item in excinfo_list)

def test_swap_flow_dot_eth_wrong_amount(backend, firmware, navigator):
    prepare_exchange(backend,firmware,navigator)
    dot = PolkadotClient(backend)
    assert dot.sign_init().status == 0x9000
    try:
...
def test_swap_flow_dot_eth_wrong_amount_then_ok(backend, firmware, navigator):
    prepare_exchange(backend,firmware,navigator)
    dot = PolkadotClient(backend)
    assert dot.sign_init().status == 0x9000
    try:
...
def test_swap_flow_dot_eth_wrong_dest_addr(backend, firmware, navigator):
    prepare_exchange(backend,firmware,navigator)
    dot = PolkadotClient(backend)    
    assert dot.sign_init().status == 0x9000
    try:
...
def test_swap_flow_dot_eth_wrong_tx_method(backend, firmware, navigator):
    prepare_exchange(backend,firmware, navigator)
    dot = PolkadotClient(backend)    
    assert dot.sign_init().status == 0x9000
    try:
...
def test_swap_flow_dot_eth_wrong_refund_addr(backend, firmware, navigator):
    prepare_exchange(backend,
                     firmware, navigator, 
                     refund_address=b"15yvXMFv8gsgxZWCmaxjFsJJfLU9qhT6qrHMoz7a9ZkTsg9A",
                     addresses_ok=False)
    
def test_swap_flow_eth_dot_payout_addr_ok(backend, firmware, navigator):
    prepare_exchange(
                    backend,
                    firmware, navigator, 
...
def test_swap_flow_eth_dot_wrong_payout_addr(backend, firmware, navigator):
    prepare_exchange(
                    backend,
                    firmware, navigator, 
                    amount=1000000000000000000,
